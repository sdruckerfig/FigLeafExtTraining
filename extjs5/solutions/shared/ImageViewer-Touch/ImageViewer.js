Ext.define('Ext.ux.panel.ImageViewer', {
	...
	initComponent: function(config) {
		...
		if (this.initOnActivate)
			this.on('activate', this.initViewer, this, {
				delay: 10,
				single: true
			});
		else
			this.on('afterrender', this.initViewer, this, {
				delay: 10,
				single: true
			});
	},
	...
	onImagePinchStart: function(ev) {
		...
		// this.getScrollable.setMomentumEasing(0);
		this.getScrollable().setDisabled(true);
	...
	applyTransform: function() {
		...
		if (Ext.is.Android) {
			...
			this.imgEl.dom.style.webkitTransform =
				'translate3d(' + fixedX + 'px, ' + fixedY + 'px, 0)' + ' scale3d(' + fixedScale + ',' + fixedScale + ',1)';
		}
	...
	adjustScroller: function() {
		...
		// apply scroll
		this.getScrollable().setX(this.scrollX || 0);
		this.getScrollable().setY(this.scrollY || 0);
